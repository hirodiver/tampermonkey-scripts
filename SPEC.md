...
### 未配信（`@version` は 3.10.1 のまま据え置き）
- **実機報告: 引用ポストで、引用した側にYouTube URLがあるのに、引用元の無関係なURL（YouTube以外）がボタンで開く不具合を修正**
- 原因は `resolveFromDom()` 最後のフォールバック「範囲内に t.co が1本だけなら採用する」。引用ポストは引用した側の本文が空のことがあり、その場合に範囲内で唯一の t.co が**引用元のもの**になって採用されていた
- 対策1: このフォールバックを `findYouTubeTextLink()` 経由に変え、**表示テキストがYouTubeに見えるリンクだけ**を候補にした。表示テキストから動画IDが完全に読めれば確定URLとして扱う
- 対策2: `findYouTubeTextLink()` が**引用ブロック内のリンクを除外**するようにした（`SELECTOR.quote`）。root自身が引用ブロックのときは除外しない（引用元側のボタンを作る経路のため）
- 検証を54項目に拡充（引用元の無関係なURLを掴まない／引用した側のリンクを正しく採る）
- **配信は保留中。** 番号を上げていないので自動更新は動かない

### v3.10.1
- **v3.10.0（引用ポスト対応）を取り消し、機能をv3.9.1の状態へ戻した。** 実装は `claude/youtube-card-quoted-posts-9dguha` ブランチに残してある
- **番号は3.9.1へ戻さず3.10.1へ進めた。** Tampermonkeyはバージョンが上がったときだけ更新するため、番号を戻すと既に3.10.0が入った端末へ自動更新でダウングレードが配信されない。巻き戻しのときも番号は必ず前進させる（→ README）

### v3.10.0（取り消し済み）
- 引用ポスト対応を追加したが、v3.10.1で取り消した。内容はコミット履歴と上記ブランチを参照
