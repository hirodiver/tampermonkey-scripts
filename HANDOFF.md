...
**それでも更新されない場合**、スクリプト個別の更新設定を疑う。
編集画面の「設定」タブに更新の可否があり、ここが「しない」だと
ダッシュボードの「更新を確認」が**そのスクリプトを黙って飛ばす**。
最終手段として、配布URLを Safari で開いて入れ直せば確実に反映される
（`@namespace` + `@name` が同一なので二重登録にはならず、
このスクリプトは `GM_setValue` を使わないので失う設定も無い）。
