...
4. ドック下の「⋮」から、置き場所（四隅）を選べる
5. ドックの一番下の矢印（‹ / ›。画面端の側を指す）が「しまう」。押すと、ドックは画面端の
   細いつまみ（‹ / ›）だけになる。つまみを押すと戻る。
...
Trusted Types を強制する CSP を付けた模擬ページで195項目を確認する。
...
個別ポストからホームへ移って切り替え、元の個別ポストへ戻る（成功・失敗・読み込みとも）／
...
読込に失敗したらドロワーを残し、止まった段階と「記録をコピー」を出し、手で開いた一覧を覚えて知らせる／元のページへ戻す／
消されたドックを作り直す、など。
