...
1. 消したい発言にカーソルを載せると、右上に2つのボタンが出る
   ...
   - **「文だけ消す」**: 名前は残し、本文だけ消す（誰の発言かは分かるようにする）。
     本文の位置には「（非表示）」が入る
   同じ相手にもう一方を押すと、二重登録ではなく消し方の切り替えになる。

   （言い回しは版によって変わっている。v1.0〜v1.3 は「非表示」「文だけ」、
   v1.4 は「発言ごと消す」「本文だけ消す」だったが、どちらも消える対象が
   一目で分からないという指摘を受け、v1.5 で「消える範囲に名前を含むか」が
   そのまま読める言い方にした）
2. 画面右上（チャット上部の見出し帯のすぐ下）の「非表示リスト」ボタンでパネルが開く。
   登録済みの一覧と、行ごとの消し方切り替え（「名前と文」/「文だけ」）・「解除」、
...
- **「文だけ消す」は本文（`#message`）とステッカー画像（`#sticker`）だけを消す**。
  名前・アイコン・金額は残す。ティッカーは本文を持たない（名前と金額だけ）ため、
  「文だけ消す」では触らず、「名前と文を消す」のときだけ消す
- v1.0 で保存した設定には消し方の区別が無い。読み込み時に「名前と文」として扱う
  （当時の挙動）
...
チャット欄の構造を模したDOMで37項目（対象だけ消える／新着・スパチャ・ティッカー／
「文だけ消す」で名前が残り本文が消える／消し方の切り替え／表示名での指定／
...
帯は消えるが、その分の空白が残ることがあった。実機の診断で出どころを絞った結果、
**消した帯を囲む要素は「表示はnoneなのに実測の高さの内訳が合わない」状態**だった
（子の高さは0pxなのに親の実測は112px、など）。DOM上は空なのに高さが残るという
ことは、CSSやDOM操作の問題ではなく、**Xの仮想リストが帯を消す前に測った高さを
まだ使い回している**可能性が高い。

そのため v2.1.0 で、**帯を新しく消したとき `resize` / `scroll` イベントを送り、
仮想リストに再計測を促す**方式にした。連打すると本来のスクロールを荒らすので、
実際に何かを新しく消したときだけ、少し待って一度だけ送る。設定の
`NUDGE_LAYOUT` で切り分け用に無効化できる。

**これは仮説であり、Xの内部実装が変われば効かなくなる。** DOM操作で空白を畳む
処理（v1.7.0〜v2.0.0で試して撤去済み）はもう入れていない。効かない場合は
`#tmspaces` の「消した帯の親の高さ」を見て、次の手を考える。
...
iPhone 相当のビューポート（390px）で33項目。帯が消える／リンクの無い帯も消える／
投稿は消えない／セル使い回しで戻る／スペースページでの復帰／診断パネル／
CSS先出し（ピルを一瞬も見せない）／ピル（同居する「新しいポストを表示」は残す／矢印だけの入れ物も帯ごと消える）／
空白対策（新規に隠したときだけ再計測イベントを送る）／診断（空白の出どころが載る）／色と形での検出（実機の紫バーが消える／紫でも小さいボタンは消えない／
