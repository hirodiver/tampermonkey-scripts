...
- 「非表示リスト」ボタンの縦位置は固定値ではなく、**見出し帯（`yt-live-chat-header-renderer` 等）の
  下端を毎回実測**して決める。帯の高さは配信ごとに変わる（メンバー章の行が増える等）ため。
  帯が見つからないときだけ `OPEN_FALLBACK_TOP`（既定56px）を使う
...
チャット欄の構造を模したDOMで33項目（対象だけ消える／新着・スパチャ・ティッカー／
「文だけ」で名前が残り本文が消える／消し方の切り替え／表示名での指定／一時解除／
ボタンが見出し帯より下に出る・帯の高さに追従する／パネルからの解除／
再読み込み後の保持）を確認する。
