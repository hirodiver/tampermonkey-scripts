...
3. 切替後は、ホームの「フォロー中」タブを開く（`OPEN_FOLLOWING_AFTER_SWITCH`。v1.4.0）。
   切替前に居たページへは戻らない
4. アイコンの下の区切り線から下が操作のボタン。記号ではなく文字で「移動」「しまう」と出す
...
すでに増えてしまった分は、X の切替メニュー（パソコンでは左下のアカウントボタン）を一度開くと、
「既存のアカウントを追加」がある一覧で置き換わって消える。
...
Trusted Types を強制する CSP を付けた模擬ページで217項目を確認する。
