...
3. 切替後は、ホームの「フォロー中」タブを開く（`OPEN_FOLLOWING_AFTER_SWITCH`。v1.4.0）。
   切替前に居たページへは戻らない
4. アイコンの下の区切り線から下が操作のボタン。記号ではなく文字で「移動」「読込」「しまう」と出す
   （「読込」は一覧があるときだけ。関係ないアカウントが入ったときなどに押すと、X の切替メニューを
   開いて一覧を読み直し、実際にログインしているアカウントだけに置き換える。v1.5.0）
...
すでに増えてしまった分は、ドックの「読込」（v1.5.0）を押すか、X の切替メニュー（パソコンでは
左下のアカウントボタン）を一度開くと、「既存のアカウントを追加」がある一覧で置き換わって消える。

v1.5.0 で、ドックの区切り線の下に「読込」を足した（要望）。一覧を先に消すのではなく、切替メニューの
一覧すべてを読めたときだけ置き換えるので、途中で失敗しても今の一覧は残る。そのため確認は出さない。
一覧が空のときはアイコン欄の大きな「読込」だけを出し、下の「読込」は隠す。
