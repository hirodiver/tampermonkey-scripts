...
帯を消しても、それを包む入れ物が高さを持ったままだと空白が残る。そこで帯の親を
6段まで遡り、**「見えている中身がもう1つも無いのに高さだけある」要素を畳む**。
タイムライン本体（`main` / `section` / `role=region`）までは遡らない。

**文字が残っていれば中身あり、アイコンだけなら名残とみなして畳む。**
実機では帯を消したあと入れ物に下向き矢印だけが残り、空白の帯として見えていた。
表示中の子要素があれば触らない作りだと、この矢印のせいで永久に畳めない。

ただし「新しいポストを表示」（`pillLabel` または `aria-label` が
`新しいポスト` / `New posts`）を含む入れ物は畳まない。帯と同居している場合は、
入れ物ごとではなく**帯（`placementTracking`）だけを消す**ので機能は残る。
この除外は CSS 側の条件にも入れてある。

X はスペースを配信（broadcasts）側へ寄せつつあるため、`/i/spaces/` に加えて
`/i/broadcasts/` へのリンクと、`audiospace` を含む `data-testid` も目印に入れている。

`:not(:has(article))` と「投稿本体を含まない」条件により、スペースに言及しただけの
投稿は巻き込まれない。非表示は削除ではなく `display:none`。X はスクロール時にセル要素を
使い回すため毎回判定し直し、帯でなくなった要素は表示へ戻す。`/i/spaces/...` を開いて
いる間はすべて表示へ戻す。

### 自動で消えないとき: タップで指定する（iPhone だけで完結）

Xの DOM を当てにいく方式には限界があるため、**指で示せば覚える**経路を用意している。

1. 帯が出ている状態で URL の末尾に `#tmspaces` を付けて開く
2. 「タップで指定」を押す
3. 消したい帯を1回タップする

タップした場所から上へ辿って帯の範囲を決め、その**居場所**（親を遡った
`data-testid` とタグの並び・`role`・`aria-label`）を `localStorage` に覚える。
文言や配信者が変わっても効き続ける。投稿本体が入ってきた要素は使い回しとみなして
消さないので、タイムラインが飛ぶことはない。間違えたら「取り消し」「全解除」。
...
iPhone 相当のビューポート（390px）で37項目。帯が消える／リンクの無い帯も消える／
投稿は消えない／セル使い回しで戻る／スペースページでの復帰／診断パネル／
CSS先出し（ピルを一瞬も見せない）／空白潰し（入れ物に高さが残らない／
中身が見えている入れ物は畳まない／矢印だけ残った入れ物も畳む／
「新しいポストを表示」を含むなら畳まない）／色と形での検出（実機の紫バーが消える／紫でも小さいボタンは消えない／
