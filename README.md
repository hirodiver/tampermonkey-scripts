...
帯を消しても、それを包む入れ物が高さを持ったまま空白の帯として残ることがある。

**自動で畳む処理は入れていない。** v1.7.0〜v1.8.0 で「中身が無いのに高さだけある
親を畳む」「アイコンだけなら名残とみなす」を試したが、実機では効かず、
模擬DOMでしか通らないコードが残っただけだった（模擬DOM側では CSS がすでに
入れ物ごと消していて、畳み込み処理を通っていなかった）。v1.9.0 で削除した。

**残った空白は下の「タップで指定」で消す。** 空白そのものをタップすればよい。
帯の名残は `data-testid` も `role` も持たないただの `div` のことがあるため、
記憶した要素を探す候補に `nav` / `header` の直下と孫も入れてある。
...
iPhone 相当のビューポート（390px）で36項目。帯が消える／リンクの無い帯も消える／
投稿は消えない／セル使い回しで戻る／スペースページでの復帰／診断パネル／
CSS先出し（ピルを一瞬も見せない）／ピル（同居する「新しいポストを表示」は残す／矢印だけの入れ物も帯ごと消える）／
残った空白をタップ指定で消す／色と形での検出（実機の紫バーが消える／紫でも小さいボタンは消えない／
