...
**URL の末尾に `#tmspaces` を付けて開く**と、画面下に診断パネルが出る
（例: `https://x.com/home#tmspaces`）。帯として検出したもの、「スペース」を含む要素の
`data-testid` / `aria-label` / 寸法 / 親チェーンが並ぶので、「コピー」を押して
そのまま報告に使える。**iPhone ではコンソールが開けない**ため、この経路を用意している。
...
iPhone 相当のビューポート（390px）で21項目。帯が消える／リンクの無い帯も消える／
投稿は消えない／セル使い回しで戻る／スペースページでの復帰／診断パネル、を確認する。
