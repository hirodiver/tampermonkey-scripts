...
1. 消したい発言にカーソルを載せると、右上に **「発言を消す」** ボタンが出る。
   押すと、その人の発言は**名前を残して本文だけ**消える（本文の位置には「（非表示）」が入る）。
   誰が喋ったかは分かるので、会話の流れは追える。
   消した後は同じ場所のボタンが **「発言を戻す」** に変わり、押すと登録が解除されて元に戻る
...
- **「発言を消す」は本文（`#message`）とステッカー画像（`#sticker`）だけを消す**。
...
「発言を消す」で名前が残り本文が消える／パネルの「まるごと消す」のオン・オフ／同じボタンで「発言を戻す」／表示名での指定／
