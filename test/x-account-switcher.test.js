...
(async () => {
  ...
  // ==========================================================
  {
    ...
    check('起動: 一覧が空のうちは、下の「更新」は出さない（アイコン欄の「読込」だけ）', await page.evaluate(() =>
    ...
    // 関係ないアカウントが入ってしまったら、下の「更新」で読み直して消す
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(400);
    check('更新: 一覧があるときは、下に「更新」ボタンが出る', await page.evaluate(() => {
    ...
    check('更新: 切替メニューを開いて読み直し、関係ないアカウントを消す（本物の3件だけ残る）',
      JSON.stringify((await stored(page)).sort()) === JSON.stringify(['alice', 'bob', 'carol']), await stored(page));
    check('更新: 読み直した後はメニューを閉じ、何も押していない', await page.evaluate(() =>
    ...
    check('更新: 覚えた件数を知らせる', /3件のアカウントを覚えました/.test(await toastText(page)), await toastText(page));
